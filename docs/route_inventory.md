...
| Method | Path | Surface | Canonical | Notes |
|---|---|---|---|---|
| GET | `/sign-in` | LiveView (`AshAuthentication.Phoenix.SignInLive`) | Yes | Auth/session route; exclude from sitemap. |
| GET | `/register` | LiveView (`AshAuthentication.Phoenix.SignInLive`) | Yes | Auth/session route; exclude from sitemap. |
| GET | `/forgot-password` | LiveView (`AshAuthentication.Phoenix.SignInLive`) | Yes | Auth/session route; exclude from sitemap. |
| GET | `/password-reset/:token` | LiveView (`AshAuthentication.Phoenix.ResetLive`) | Yes | Tokenized auth flow; exclude from sitemap. |
| GET | `/confirm-new-user/:token` | LiveView (`AshAuthentication.Phoenix.ConfirmLive`) | Yes | Tokenized auth flow; exclude from sitemap. |
...
## Canonical redirects (current)

- None currently implemented.

Planned canonical redirects once catalog routes exist:
- `/shop` -> `/products`
- `/catalog` -> `/products`
