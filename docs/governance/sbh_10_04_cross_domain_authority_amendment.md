...
InventoryAdmission's typed Request derives the generic key, hashes the exact key into logical identity, and rejects a caller-supplied key. Operation descriptors carry that key. The current Lease value carries its identity digest. S0-ARCH-01 specifies PostgreSQL reconciliation by the exact `reservation_key`; Redis remains a bounded coordination and fencing layer. See [`request.ex`](../../lib/store/orders/inventory_admission/request.ex), [`operation.ex`](../../lib/store/orders/inventory_admission/operation.ex), [S0-ARCH-01](../hardening/s0_inventory_reservation_admission_architecture.md), and [the S0 implementation plan](../hardening/s0_inventory_reservation_admission_implementation_plan.md).

At this base, the full InventoryAdmission PostgreSQL recovery worker and recovery service remain planned work. This amendment proves that the accepted identity contract can carry an exact generation key without changing PostgreSQL authority. It does not certify runtime recovery as complete. The later SUBS re-admission must verify the then-current S0 source and must stop if the required recovery path is absent or needs authority beyond this grant.
...
The collection resource requires one task-specific Subscription migration and its Ash snapshot. It must not repurpose `RenewalAttempt.attempt_no` or redefine the legacy `RenewalAttempt.payment_intent_id`. That field remains unresolved compatibility/state evidence until the separate SBH-10-05 decision.
...
For a physical renewal, the same transaction must consume only the validated exact generation keys through a bounded Orders API before or alongside the paid-Order transition. The collection-to-generation relation is established from the durable collection/generation IDs and the server-derived reservation keys; an order-wide active-hold assumption is insufficient. The collection validator must run before the existing payment-success path can consume reservations for the Order. The renewal path must use the exact-key consumption operation, or prove the active-key set is exactly equal before any order-wide consume. Generic checkout keeps its existing path.
...
|---|---|---|
| Subscriptions | One `RenewalCollectionAttempt` resource, one task-specific migration and Ash snapshot, collection concurrency, dispatch/outcome evidence, and exact PaymentIntent and reservation-generation links. Renewal retries read finalized totals from the existing Order. | One immutable RenewalAttempt and one Order remain. Dunning policy and the legacy `payment_intent_id` meaning do not change. |
| Payments | A typed renewal create-or-reuse path using the collection-derived key, plus bounded `requires_action` handling and renewal success routing through Subscription validation and exact-key Orders consumption. | Reuse PaymentIntent and PaymentApplication. No new Payments resource or schema migration. Generic checkout remains unchanged. |
...
| InventoryAdmission / S0 | Extend the server-derived identity and recovery contract to the exact renewal generation key. | Preserve `K_v`, `B_total`, PostgreSQL authority, lease safety, recovery fencing, and fail-closed behavior. No Redis business authority or redesign. |
...
- **Warm:** No new ETS or Redis cache is authorized. Stock and availability projections remain read-only; PostgreSQL decides reservation and stock truth. Existing TTL and post-commit invalidation behavior stays unchanged. No cache stampede protection is added to this admission path; database row locks and uniqueness constraints remain the concurrency controls.
...
- **Indexes:** Retain unique `payment_intent_key`, the existing partial same-Order active-payment index, unique `reservation_key`, and add the authorized active-reservation partial index. The Subscription migration must add unique `(renewal_attempt_id, collection_ordinal)` and the partial one-active-collection index.
