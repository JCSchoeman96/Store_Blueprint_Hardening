...
7. The grant covers `Store.Subscriptions.RenewalAttempt`, minimum `Store.Subscriptions.Facade` bind orchestration, the one governed `Store.Subscriptions.ContractChange` transition, Subscription aggregate/version inspection needed for authoritative binding, and focused fixtures/tests. The exclusions in the master register remain in force.
8. The implementation branch and `task_base_sha` remain unassigned until this governance PR merges and the human owner verifies the exact merge commit and tree. The accepted SUBS tip then becomes the prospective base for JC-228 / SBH-10-03.
...
3. Implement the minimum facade transaction that reads current Subscription/version and exact current target, freezes the complete occurrence, marks the exact ContractChange `BOUND_TO_RENEWAL`, and removes the current unbound target in one commit.
...
5. Run focused Subscription tests, migration and snapshot checks, formatting, and `mix check`; record actual results in the implementation PR. Create the named implementation branch only after the governance merge and exact-tree verification.
