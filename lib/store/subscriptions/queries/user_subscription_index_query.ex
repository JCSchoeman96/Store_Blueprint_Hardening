defmodule Store.Subscriptions.Queries.UserSubscriptionIndexQuery do
  @moduledoc """
  Typed user subscription listing query.
  """
  ...
  @enforce_keys [:status, :plan_key, :limit, :offset]
  defstruct status: nil, plan_key: nil, limit: 20, offset: 0
  ...
  def new(params) when is_map(params) do
    with :ok <- validate_keys(params),
      ...
      {:ok, %__MODULE__{status: status, plan_key: plan_key, limit: limit, offset: offset}}
    end
  end
  ...
  defp validate_keys(params) do
    ...
    case unknown do
      [] -> :ok
      _ -> {:error, Error.new("VALIDATION_ERROR", "unknown params: #{Enum.join(unknown, ", ")}")}
    end
  end
  ...
  defp parse_optional_status(params) do
    case Map.get(params, :status) || Map.get(params, "status") do
      ...
      value when is_binary(value) ->
        value |> String.trim() |> String.downcase() |> String.to_atom() |> validate_status()
  ...
  defp fetch_int(params, key, default) do
    case Map.get(params, key) || Map.get(params, Atom.to_string(key)) do
      ...
      _ -> {:error, Error.new("VALIDATION_ERROR", "#{key} must be an integer")}
    end
  end
